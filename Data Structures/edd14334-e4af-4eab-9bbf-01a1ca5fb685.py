...
if not re.search('\s*for\s*{}\s*in\s*capitals.keys\s*\(\s*\)\s*:\s*print\s*\(\s*{}\s*\)\s*'.format(variable, variable), code_string):
    print("Please, loop through keys of capitals and print keys", file=sys.stderr)
    sys.exit(0)
