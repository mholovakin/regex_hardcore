...
if not is_print:
    print("Please, compare giraffe_age with elephant_age and giraffe_age with lion_age", file=sys.stderr)
    sys.exit(0)
...
if re.search('(\s*lion_age\s*(>=|>|<=|<)\s*giraffe_age)|(\s*giraffe_age\s*(>=|>|<=|<)\s*lion_age\s*)', code_string):
    is_giraffe_lion = True

is_giraffe_elephant = False
if re.search('(\s*elephant_age\s*(>=|>|<=|<)\s*giraffe_age)|(\s*giraffe_age\s*(>=|>|<=|<)\s*elephant_age\s*)', code_string):
    is_giraffe_elephant = True

if not (is_giraffe_lion and is_giraffe_elephant):
    print("Please, compare giraffe_age with elephant_age and giraffe_age with lion_age", file=sys.stderr)
    sys.exit(0)

passed = False
for line in user_code_lines:
    if re.match('\s*print\s*\([a-z0-9><=].*\s*and\s*[a-z0-9<>=].*', line):
        passed = True

if not passed:
    print("Please, print the comparison of variables with \"and\" operator", file=sys.stderr)
    sys.exit(0)
